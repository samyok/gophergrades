...
def process_class(x):
    ...
    dept = session.query(DepartmentDistribution).filter(DepartmentDistribution.dept_name == dept_name).first()
    if class_dist == None:
        if dept == None:
            dept = DepartmentDistribution(dept_name=dept_name)
            session.add(dept)
            session.flush()
        class_dist = ClassDistribution(class_name=class_name,total_students=num_students,total_grades=grade_hash,department_id=dept.id)
